...
export default function PixKeys() {
  ...
  return (
    <div className="p-2 md:p-6 animate-fade-in">
      <h1 className="section-title mb-4">Minhas Chaves Pix - Personalizado por Você</h1>
      <form onSubmit={handleSave} className="mb-6 grid grid-cols-1 md:grid-cols-5 gap-2">
        <input className="input-modern" placeholder="Banco" value={form.bank ?? ''} onChange={e => setForm(f => ({ ...f, bank: e.target.value }))} />
        <input className="input-modern" placeholder="Titular" value={form.owner ?? ''} onChange={e => setForm(f => ({ ...f, owner: e.target.value }))} />
        <input className="input-modern" placeholder="Tipo (Email, Telefone, CNPJ)" value={form.type ?? ''} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} />
        <input className="input-modern" placeholder="Chave Pix" value={form.value ?? ''} onChange={e => setForm(f => ({ ...f, value: e.target.value }))} />
        <input className="input-modern" placeholder="Instruções" value={form.instructions ?? ''} onChange={e => setForm(f => ({ ...f, instructions: e.target.value }))} />
      ...
      <div className="overflow-x-auto card">
        <table className="table-modern">
          ...
          <tbody>
            {keysToShow.map(key => (
              <tr key={key.id}>
                <td className="px-4 py-2 font-semibold text-gray-900">{key.bank}</td>
                <td className="px-4 py-2 text-gray-700">{key.owner}</td>
                <td className="px-4 py-2 text-gray-700">{key.type}</td>
                <td className="px-4 py-2 text-indigo-700 font-bold">{key.value}</td>
                <td className="px-4 py-2 text-gray-500">{key.instructions}</td>
